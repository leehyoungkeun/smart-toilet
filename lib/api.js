...
export async function recordCleaning(pin, cleaner, note) {
  const { data, error } = await supabase.rpc('record_cleaning', { p_pin: pin, p_cleaner: cleaner || null, p_note: note || null });
  if (error) throw error;
