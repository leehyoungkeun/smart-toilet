...
// 하드웨어(MCU)가 읽은 센서값 업로드 — 서버가 current_location 으로 화장실 결정
export async function recordEnvReading(temp, humidity, ammonia, pm25) {
  const { error } = await supabase.rpc('record_env_reading', {
    p_temp: temp, p_humidity: humidity, p_ammonia: ammonia, p_pm25: pm25,
  });
  if (error) throw error;
}
